...
app.get("/api/ticket/:id", async (req, res) => {
  ...
  try {
    ...
    const messages = (m.data || []).map((x) => ({
      ...
      text: x.stripped_text || x.body_text || stripHtml(x.stripped_html || x.body_html),
      at: x.created_datetime, internal: x.channel === "internal-note" || x.public === false,
    ...
    res.json({
      ...
      tags: (t.tags || []).map((x) => x.name), emily: emilyStatusOf(t), category: categorize(t), messages,
    });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
